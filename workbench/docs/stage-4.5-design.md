...
### D4.5-1 画布导航（P0，最高优先级）

- 滚轮缩放，**以鼠标指针位置为缩放中心**（不是画布中心）；触控板捏合天然落在 ctrl+wheel 事件上，顺带支持
- 空白处拖拽 = 平移画布（节点上拖拽 = 移动节点，互不冲突，按事件目标区分）
- 双击空白处 = 回到全图（fit）
- 小地图与视口实时联动（小地图已有，接视口矩形）
- **引擎层实现**，工作台与离线 HTML 同享
- ❗ 与钉扎无冲突的实现要点：钉扎存的是**模型坐标**，视口缩放平移只是观察矩阵变换，两者天然正交——实现时不要把视口偏移混进节点坐标
- 不做：惯性滚动、缩放动画曲线调优（先把基础做对）
...
1. **导航**：滚轮以指针为中心缩放；空白拖拽平移；双击空白回全图；小地图框联动；**离线 HTML 双击打开后同样全部可用**
