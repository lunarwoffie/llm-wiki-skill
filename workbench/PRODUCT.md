...
- Step 3：活模拟 + 钉扎（松手即钉/双击解钉）+ 持久化（`.wiki-graph-layout.json`，只存钉的、路径为 key）
...
**完整设计**：[docs/stage-4-design.md](docs/stage-4-design.md)（D1–D14 关键决策 + 8 Step 细则 + API 契约 + 验收剧本 + 风险清单）

### 阶段 4.5：图谱可用性收尾 🚧 设计完成 2026-06-13，待实施
...
- P0 点击语义重构："点击即阅读，选区即升级"——单击 = 右抽屉阅读态，选区悬浮窗取消，抽屉一容器两状态；动作映射表补"单节点"行并修复 unlinked 误触发（修订 stage-4 D6）
...
**完整设计**：[docs/stage-4.5-design.md](docs/stage-4.5-design.md)（D4.5-1~9 + 动作映射表完整版 + 抽屉去留表 + 验收剧本）
...
#### 图谱演进候选池（2026-06-13 全景分析沉淀，4.5 之后再排期）
...
**推荐切片（4.5 验收后的下一图谱阶段，全部高价值低成本）**：

| 项 | 一句话 | 为什么值得 |
|---|---|---|
| 局部图模式 | 点节点"聚焦"只显示 1~2 跳邻域（跳数可调），其余淡出 | 接住教训一：大库日常视图应是局部图；与"+邻居"选区天然衔接 |
...
| 关系类型上边 | 按关系词汇表（实现/依赖/对比/矛盾）给边虚实/颜色，"矛盾"边标红 | SKILL.md 关系词汇表**数据已有**，图上从未用过，纯 UI 接线 |
| 类型/时间过滤器 | 实体/主题/来源三个开关 + "只看最近 7 天"；不做查询语法 | 轻量；Obsidian 式语法属于过度设计 |
| 导出美图 | 当前视角一键导出带主题样式的 PNG/SVG | 数字山水是最强传播资产，用户发图 = 免费获客，成本极低 |
| 图谱增强检索（后端暗改） | ADR-19 检索命中页 A 时顺带取一跳邻居 + 社区信息进上下文 | GraphRAG 核心思路：图谱不只给人看，是 agent 的检索结构；半天级工作量 |
...
- ❌ WebGL 渲染重写（个人库 99% 在千节点内；语义缩放+局部图让视口内永远只有几十个可读节点，比换渲染器治本；且会杀死手绘质感。R6 的 canvas 后门保留即可）
...
**决策**（细节与论证见 [docs/stage-4-design.md](docs/stage-4-design.md) D1–D14）：
...
3. **活模拟 + 钉扎**：d3-force（单模块）；预计算起点 + 低温入睡的混合布局；拖动低温让位、松手即钉、双击解钉；钉扎存知识库根 `.wiki-graph-layout.json`（只存钉的、库内相对路径为 key、模型坐标）。原则：**对知识的主观组织进库文件，浏览状态留本机**
4. **位置层/结构层分权**：拖动只改位置，颜色/社区/连线永远由真实 wikilink 决定；想改结构 → 通过选区提问让 agent 建链写回 wiki。图谱永不撒谎
5. **选区 = 批量 `@`**：结构化四式选择（点节点/点社区/+邻居/Shift 多选），砍自由套索（空间邻近无语义保证）；选区面板结构事实先行、动作随性质变；动作本质是已有工作流（digest/comparisons/lint/crystallize）的空间入口；沿用 `/api/prompt` 文本通道不加新参数
...
**何时重新评估**：
- 力模拟在真实大库（500+ 节点）实测掉帧 → 布局计算挪 Web Worker，再不够才考虑 canvas 后端
...
4. **画布导航是图谱的地基能力**（缩放/平移/回全图/小地图联动），引擎层实现两端同享

**与既有 ADR 的关系**：修订 ADR-21 第 5 条的选区面板形态（悬浮窗 → 抽屉态）；强化 ADR-2（对话中心，图谱阅读复用工作台抽屉基建）；沿用 stage-4 D9 原则（视口/图例折叠等浏览状态留本机）。细节见 [docs/stage-4.5-design.md](docs/stage-4.5-design.md)。
...
**设计文档**：[docs/stage-4-design.md](docs/stage-4-design.md)（D1–D14 关键决策 + 8 Step + API 契约 + 验收剧本 + 风险/TBD + 明确不做清单）
...
### 阶段 4.5：图谱可用性收尾 🚧 设计完成 2026-06-13

**当前状态**：设计完成（主仓库 `stage-4` 分支），待写实施 plan 后交 codex 执行。

**设计文档**：[docs/stage-4.5-design.md](docs/stage-4.5-design.md)（D4.5-1~9 决策 + 动作映射表完整版 + 抽屉去留表 + 验收剧本 + 风险）；决策记录：ADR-22。
...
- **2026-06-13 v16（阶段 4.5 设计完成）**：图谱可用性收尾设计定稿
  ...
  - §10 新增阶段 4.5 小节；设计细则归档 [docs/stage-4.5-design.md](docs/stage-4.5-design.md)
...
- **2026-06-12 v14（阶段四设计完成）**：monorepo 合并 + 图谱活地图设计定稿
  ...
  - 设计细则归档 [docs/stage-4-design.md](docs/stage-4-design.md)（D1–D14 决策 + API 契约 + 验收剧本 + 风险/TBD + 明确不做清单）
