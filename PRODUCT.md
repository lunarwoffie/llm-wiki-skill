...
### 阶段四：monorepo 合并 + 图谱活地图 🚧 设计完成 2026-06-12，待实施
...
### 阶段四：monorepo 合并 + 图谱活地图 🚧 设计完成 2026-06-12

**当前状态**：设计完成（分支 `stage-4-design`），待作者审定后实施。实施场地为**主仓库**（Step 0 起开发主场切到 monorepo）。
...
**关键风险**（详见设计文档 §7）：R1 根 package.json 不设 type:module（主仓库 CommonJS 测试）；R2 手绘路径帧缓存（沸腾效应）；R3 fs.watch recursive 实测先行；R4 subtree 隐私路径检查。
